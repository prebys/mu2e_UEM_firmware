...
//
  ...
  hprof2d_vpeaksum_coin_us_kicker1 = new TProfile2D("hprof2d_vpeaksum_coin_us_kicker1","vprofile 2D peaksum coin_us kiker1 ",3000,155000,158000,10000,0,10000,-30000,0);

  hprof2d_waterfall = new TProfile2D("hprof2d_waterfall","waterfall ",5000,0,5000,240,0,240,-30000,0);
  ...
  //for(int i= 0; i< 10 ;i++)
   { 
       ...
       for(int k=0; k<nsum[i] ; k++)
	{
           ...
           hprof2d_vpeaksum_coin_us_kicker1 -> Fill(ch_vpeaksum_time1_us[i][k],i,ch_vpeaksum[i][k],1);
...
double time_trace;
  for(int i= 0; i< nevn ;i++)
  {
    for(int itrace = 0; itrace < 700; itrace++)
     {
       time_trace = 600 + itrace*278.25; //25*11.13 = 278.25
...
// 
   ...
   TCanvas *c_hprofile_peaksum_kicker1 = new TCanvas("c_hprofile_peaksum_kicker1","profile kicker1 peak area",200,10,700,500);
//  
...
//   TCanvas *c_sum_ratio_tail_total = new TCanvas("c_sum_ratio_tail_total"," (tail / total) vs total",200,10,700,500);
   ...
   TCanvas *c_hprof2d_waterfall = new TCanvas("c_hprof2d_waterfall","waterfall histogram",200,10,700,500);
     ...
     c_hprofile_peaksum_kicker1->cd();
     hprof2d_vpeaksum_coin_us_kicker1 ->GetXaxis()->SetTitle("Time(us)");
     hprof2d_vpeaksum_coin_us_kicker1 ->GetYaxis()->SetTitle("Acc. Trig. Events");
     hprof2d_vpeaksum_coin_us_kicker1->Draw("colz");
