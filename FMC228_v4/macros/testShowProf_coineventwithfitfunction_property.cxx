...
void testShowProf_coineventwithfitfunction_property(char* rootfile, int nevn)
{
  ...
  //
  //TTree *tr = (TTree*)fcoinfitfunction->Get("cointree");
  TTree *tr = (TTree*)fcoinfitfunction->Get("cointreefitfunction");

  //CoinEvent *coineventfitfunction = new CoinEvent();
  ...
  int N_four_peak = 0;
  int N_five_peak = 0;
  tr->SetBranchAddress("ch2.",&coineventfitfunction);
  ...
  //for(int i= 0; i< 10 ;i++)
   { 
       ...
       //for(int k=0; k<10 ; k++)
	{
	   ...
	   //for(int ipeak = 0; ipeak < 10; ipeak++)
	     {
	        ...
	        if(ch_vpeaksum_time_ns[i][3][k] <= ch_vpeakhigh_time_ns[i][3][ipeak] 
		   && ch_vpeakhigh_time_ns[i][3][ipeak] <= ch_vpeaksum_time2_ns[i][3][k]
		   && ch_vpeaksum_time_ms[i][3][k] <100)
	         {
       ...
       npeaksum->Fill(nsum_ch3[i]);//nsum_10ms);
       delta_peak_sum_10ms = npeak_ch3[i] - nsum_ch3[i] ;//npeak_10ms - nsum_10ms;
       ndelta_peak_sum -> Fill(delta_peak_sum_10ms);
       peak2f_high_sum -> Fill(npeak_ch3[i],nsum_ch3[i]); //npeak_10ms, nsum_10ms);
       //if(npeak_10ms != 0 && nsum_10ms !=0) {
       if(npeak_ch3[i] != 0 && nsum_ch3[i] !=0) {
           peakh1_high_sum -> Fill(npeak_ch3[i]/nsum_ch3[i]);
