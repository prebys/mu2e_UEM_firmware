...
void fmc228_data_peakstream_convert_root_v10_2024() {
  ...
  filein.open("/home/kujiwado/WorkDir/FileTransfer/mu2euem/5000evn_r3.dat",ios::in | ios::binary);
  if(!filein){
  ...
  while (!filein.eof()){
    ...
    if((buf[0] == static_cast<char>(0xff))
        ...
        filein.read(buf,4);
        //printf("buf0 %2.2x \t buf1 %2.2x \t buf2 %2.2x \t buf3 %2.2x \n", buf[0],buf[1],buf[2],buf[3]);
	while(buf[1] == static_cast<char>(0xff))
	  {//printf("header sdgfsgsg\n",buf[1]);
	  }
