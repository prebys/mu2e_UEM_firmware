...
void testShowProf_coineventfitfunction_psd_time(char* rootfile, int nevn)
{
  ...
  Int_t y[5000];
  Int_t peakhigh[5000];
  Int_t vpeakhigh[5000];
  Int_t vpeaksum[5000];
  Int_t peaktime[5000];
  Int_t peaksum[5000];
  Int_t peaksumtime1[5000];
  Int_t peaksumtime2[5000];


  Int_t ch_vpeakhigh[5000][4][1000];
  Int_t ch_vpeakhigh_time_ms[5000][4][1000];
  Int_t ch_vpeakhigh_time_us[5000][4][1000];
  Int_t ch_vpeakhigh_time_ns[5000][4][1000];

  Int_t ch_vpeaksum[5000][4][1000];
  Int_t ch_vpeaksum0[5000][4][1000];
  Int_t ch_vpeaksum_tail[5000][4][1000];
  Int_t ch_vpeaksum_peak[5000][4][1000];
  Int_t ch_vpeaksum_time_ms[5000][4][1000];
  Int_t ch_vpeaksum_time_us[5000][4][1000];
  Int_t ch_vpeaksum_time_ns[5000][4][1000];

  Int_t npeak_ch0[5000];
  Int_t npeak_ch1[5000];
  Int_t npeak_ch2[5000];
  Int_t npeak_ch3[5000];
  ...
  hprof2d_vpeaksum_coin_us_batch1 = new TProfile2D("hprof2d_vpeaksum_coin_us_batch1","vprofile 2D peaksum coin_us batch1  ",3000,0,3000,5000,0,5000,-30000,0);
  hprof2d_vpeaksum_coin_us_batch2 = new TProfile2D("hprof2d_vpeaksum_coin_us_batch2","vprofile 2D peaksum coin_us batch2 ",3000,66000,69000,5000,0,5000,-30000,0);
  hprof2d_vpeaksum_coin_us_batch3 = new TProfile2D("hprof2d_vpeaksum_coin_us_batch3","vprofile 2D peaksum coin_us batch3 ",3000,266700,269700,5000,0,5000,-30000,0);
  hprof2d_vpeaksum_coin_us_batch4 = new TProfile2D("hprof2d_vpeaksum_coin_us_batch4","vprofile 2D peaksum coin_us batch4 ",3000,333330,336330,5000,0,5000,-30000,0);
  ...
  //for(int i= 0; i< 10 ;i++)
   { 
	printf("event %d \n",i);
        ...
        //printf("event %d \n",i);
        //printf("ch3  \n");
        npeak_ch3[i] = coineventfitfunction->GetVPeakHighSize();
        nsum_ch3[i] = coineventfitfunction->GetVPeakSumSize();
	...
	{
           ch_vpeaksum[i][3][k]=coineventfitfunction->GetVPeakSum()[k];
           ch_vpeaksum0[i][3][k]=coineventfitfunction->GetVPeakSum0()[k];
           //ch_vpeaksum_tail[i][3][k]=coinevent->GetVPeakSum_tail()[k];
           ch_vpeaksum_peak[i][3][k]=coineventfitfunction->GetVPeakSum0_peak()[k];
           ch_vpeaksum_tail[i][3][k]=ch_vpeaksum[i][3][k] - ch_vpeaksum0[i][3][k];
           ratio = ( (ch_vpeaksum_tail[i][3][k]*1.0)/(ch_vpeaksum[i][3][k]*1.0) );
           //printf("tail = %d \n",ch_vpeaksum_tail[i][3][k]);
           //printf("sum = %d \n",ch_vpeaksum[i][3][k]);
           //printf("peak = %d \n",ch_vpeaksum_peak[i][3][k]);
           //printf("ratio = %lf \n",ratio);
           ch_vpeaksum_time_ms[i][3][k]=coineventfitfunction->GetVPeakSumTime1()[k]*0.000001; //convert to ms
           ch_vpeaksum_time_us[i][3][k]=coineventfitfunction->GetVPeakSumTime1()[k]*0.001; //convert to us
           ch_vpeaksum_time_ns[i][3][k]=coineventfitfunction->GetVPeakSumTime1()[k]; //ns
           ...
           hprof2d_vpeaksum_coin_us_batch4 -> Fill(ch_vpeaksum_time_us[i][3][k],i,ch_vpeaksum[i][3][k],1);
 ...
 int time_max = 70000;
 int time_min = 100;
   //for(int itime= time_min; itime <time_max; itime ++)
   //{
   //  ntrack = 0;
   //  for(int ievent=0; ievent < nevn; ievent ++)
   //   {
   //     for(int isum=0; isum < nsum_ch3[ievent]; isum ++)
   //      {
   //          if(ch_vpeaksum_time_us[ievent][3][isum] > (itime -1.5) && ch_vpeaksum_time_us[ievent][3][isum] <(itime +1.5) )
   //           {
   //               ntrack = ntrack +1;
   //          
   //               //if(ntrack > 200)
   //               //{
   //               //    //InTime[3][iInTime]= itime;
   //               //    //iInTime = iInTime+1;
   //               //    break;
   //               //}
   //          
   //          }else if(ch_vpeaksum_time_us[ievent][3][isum] >(itime+1.5) )
   //          {
   //              break;
   //          }                    
   //      }
   ...
   //   }
   //  //printf("iInTime %d\n", iInTime);
   //  //printf("itime =%d \t ntrack =%d\n",itime,ntrack);
   //}
 
  //printf("iInTime =%d\n", iInTime);
  //for(int jt = 0; jt < iInTime; jt ++)
  //{
  //  printf("InTime[3][%d] = %d \n",jt,InTime[3][jt]);
  //} 
  ...
  //  for(int isum=0; isum < nsum_ch3[ievent]; isum ++)
  //    {
  //      for(int jtime =1; jtime <iInTime-1; jtime ++)
  //       {
  //        if(ch_vpeaksum_time_us[ievent][3][isum] >time_max) break;
  //        if( ch_vpeaksum_time_us[ievent][3][isum] > (InTime[3][jtime] -0.5) && ch_vpeaksum_time_us[ievent][3][isum] < (InTime[3][jtime] + 0.5)  
  //            //&& ch_vpeaksum_time_us[ievent][3][isum] < 100 
  //          )
  //           {
  //             sum_tail_peak_high -> Fill(ch_vpeaksum_peak[ievent][3][isum],(ch_vpeaksum_tail[ievent][3][isum]*1.0)/(ch_vpeaksum[ievent][3][isum]*1.0) );
  //             sum_tail_total -> Fill(ch_vpeaksum[ievent][3][isum],ch_vpeaksum0[ievent][3][isum]);
  //             sum_ratio_tail_total -> Fill(ch_vpeaksum[ievent][3][isum],(ch_vpeaksum_tail[ievent][3][isum]*1.0)/(ch_vpeaksum[ievent][3][isum]*1.0) );
  //           }
  //        if( ch_vpeaksum_time_us[ievent][3][isum] < (InTime[3][jtime] -0.5) && ch_vpeaksum_time_us[ievent][3][isum] > (InTime[3][jtime-1] + 0.5)  
  //            //&& ch_vpeaksum_time_us[ievent][3][isum] < 100 
  //          )
  //           {
  //               bg_sum_tail_peak_high -> Fill(ch_vpeaksum_peak[ievent][3][isum],(ch_vpeaksum_tail[ievent][3][isum]*1.0)/(ch_vpeaksum[ievent][3][isum]*1.0));
  //               printf("time out-of-time proton %d ievent %d \n", ch_vpeaksum_time_us[ievent][3][isum], ievent);
  //           }
  //       }
  //    }
  //  }
   ...
   TCanvas *c3_4_sum = new TCanvas("c3_4_sum","profile",200,10,700,500);
