...
from python_analysis.event_types import *
...
class EventType:
    ...
    def __init__(self,
        ...
        self._name = name  # name of the variable of this instance
...
class Event:
    ...
    def process_hex_raw_data(self, mode, hex_in: str = "") -> ADCTypes:
        ...
        assert self.type.name in allowed_types, f"Expected one of {allowed_types}, got {self.type}"
        if not hex_in:
            hex_in = self.hex
...
class NewEvent:
    ...
    def __init__(self, internal_event_number, data_str):
        ...
        print(f"Parsing NewEvent #{internal_event_number}, data length {len(data_str)}.")
        print(f"data_str starts with {data_str[:20]}")
        if data_str.startswith('ffffff11'):
            ...
            print(f"Found timestamp: {self.timestamp}, skipping {data_str[:24]}")
        ...
        if self.timestamp:
            print(f"Event {self.event_number}-{self.internal_event_number} has timestamp {self.timestamp}")
        else:
            print(f"Event {self.event_number}-{self.internal_event_number} has no timestamp.")
